...
class Post_Type_Registry_Test extends TestCase {
	...
	private MockObject $taxonomy_mock;
	private MockObject $meta_box_collection_mock;
	private MockObject $taxonomy_collection_mock;
	...
	private MockObject $register_taxonomy_mock;
	...
	public function set_up(): void {
		...
		$this->taxonomy_mock                 = $this->getMockBuilder( Abstract_Taxonomy::class )->getMock();
		$this->meta_box_collection_mock      = $this->getMockBuilder( Meta_Box_Collection::class )->getMock();
		$this->taxonomy_collection_mock      = $this->getMockBuilder( Taxonomy_Collection::class )->getMock();
		...
		$this->register_taxonomy_mock        = $this->getFunctionMock( __NAMESPACE__, 'register_taxonomy' );
		...
		$this->taxonomy_collection_mock->method( 'getIterator' )
									   ->will( $this->returnValue( new ArrayIterator( array( $this->taxonomy_mock ) ) ) );
	}
	...
	public function should_register_post_type_meta_boxes(): void {
		$this->add_action_mock->expects( $this->once() )
							  ->with( 'init', $this->isType( 'callable' ) )
							  ->will( $this->returnCallback( fn( $action, $callable ) => $callable() ) );
		$this->register_post_type_mock->expects( $this->once() )
									  ->with(
										  'custom',
										  $this->callback(
	 ...
	 * @test
	 */
	public function should_register_taxonomies(): void {
		$this->add_action_mock->expects( $this->once() )
							  ->with( 'init', $this->isType( 'callable' ) )
							  ->will( $this->returnCallback( fn( $action, $callable ) => $callable() ) );
		$this->register_taxonomy_mock->expects( $this->once() )
									 ->with(
										 'custom',
										 'custom',
										 $this->logicalAnd(
											 ...
											 $this->arrayHasKey( 'show_in_rest' ),
											 $this->arrayHasKey( 'rest_base' ),
											 $this->arrayHasKey( 'rest_namespace' ),
											 $this->arrayHasKey( 'rest_controller_class' ),
											 $this->arrayHasKey( 'show_tag_cloud' ),
											 $this->arrayHasKey( 'show_in_quick_edit' ),
											 $this->arrayHasKey( 'show_admin_column' ),
											 $this->arrayHasKey( 'capabilities' ),
											 $this->arrayHasKey( 'rewrite' ),
											 $this->arrayHasKey( 'query_var' ),
											 $this->arrayHasKey( 'default_term' ),
											 $this->arrayHasKey( 'sort' )
										 )
									 );

		$this->post_type_mock->method( 'key' )->will( $this->returnValue( 'custom' ) );
		$this->post_type_mock->method( 'taxonomies' )->will( $this->returnValue( $this->taxonomy_collection_mock ) );
		$this->taxonomy_mock->method( 'key' )->will( $this->returnValue( 'custom' ) );
